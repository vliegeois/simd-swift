...
public struct simd_quatf: Equatable, Sendable {
    ...
    public init(angle: Float, axis: SIMD3<Float>) {
        ...
        let s = Float.sin(angle/2.0)
        let axis = simd_normalize(axis)
        vector = .init(axis.x * s, axis.y * s, axis.z * s, Float.cos(angle/2.0))
    }
    ...
    public init(from: SIMD3<Float>, to: SIMD3<Float>) {
        ...
        if SimdSwift.length(initialDirection) < 1.0e-6 || SimdSwift.length(finalDirection) < 1.0e-6 {
            self.init(angle: .zero, axis: SIMD3<Float>(1.0, 0.0, 0.0))
            return
        }
    ...
    public init(_ rotMat: float3x3) {
        ...
        if abs(rotMat[0, 1] - rotMat[1, 0]) < 0.001 && abs(rotMat[0, 2] - rotMat[2, 0]) < 0.001 && abs(rotMat[1, 2] - rotMat[2, 1]) < 0.001 {
            ...
            if xx > yy && xx > zz { // r11 is the largest diagonal
                if xx < 0.001 {
                    axis = SIMD3<Float>(0, invsqrt2, invsqrt2)
                } else {
                    let x = Float.sqrt(xx)
                    axis = SIMD3<Float>(x, xy/x, xz/x)
                }
            } else if yy > zz { // r22 is the largest diagonal
                if yy < 0.001 {
                    axis = SIMD3<Float>(invsqrt2, 0, invsqrt2)
                } else {
                    let y = Float.sqrt(yy)
                    axis = SIMD3<Float>(xy/y, y, yz/y)
                }
            } else { // r33 is the largest diagonal
                if zz < 0.001 {
                    axis = SIMD3<Float>(invsqrt2, invsqrt2, 0)
                } else {
                    let z = Float.sqrt(zz)
                    axis = SIMD3<Float>(xz/z, yz/z, z)
                }
            }
            ...
            let angle = Float.acos(cosVal)
            let den2 = Float.pow(rotMat[2, 1] - rotMat[1, 2], 2) + Float.pow(rotMat[0, 2] - rotMat[2, 0], 2) + Float.pow(rotMat[1, 0] - rotMat[0, 1], 2)
            let den = Float.sqrt(den2)
            let f = 1.0 / den
            let x = f * (rotMat[1, 2] - rotMat[2, 1])
            let y = f * (rotMat[2, 0] - rotMat[0, 2])
            let z = f * (rotMat[0, 1] - rotMat[1, 0])
            let axis = SIMD3<Float>(x, y, z)
            self.init(angle: angle, axis: axis)
        }
    }
    
    // Construct a quaternion from `rotationMatrix`.
    public init(_ rotationMatrix: float4x4) {
        let cols = rotationMatrix.columns
        self.init(float3x3(SIMD3(x: cols.0.x, y: cols.0.y, z: cols.0.z), SIMD3(x: cols.1.x, y: cols.1.y, z: cols.1.z), SIMD3(x: cols.2.x, y: cols.2.y, z: cols.2.z)))
    }
    ...
    public var imag: SIMD3<Float> { .init(vector.x, vector.y, vector.z) }
    ...
    public var angleAxis: (angle: Float, axis: SIMD3<Float>) {
    //    Q = cos(theta/2) + sin(theta/2) * v
...
extension float3x3 {
    public init(_ quatd: simd_quatf) {
        let q0 = quatd.real
        let qx = quatd.imag.x
        let qy = quatd.imag.y
        let qz = quatd.imag.z
...
extension float4x4 {
    public init(_ quatf: simd_quatf) {
        let rot = float3x3(quatf)
