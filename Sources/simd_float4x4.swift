...
public struct simd_float4x4: Equatable, Sendable {
    ...
    internal init(_ floats: [Float]) {
        guard floats.count >= 16 else {
            self = .init(floats.fill(.zero, to: 16))
            return
        }
        columns = (.init(x: floats[0], y: floats[1], z: floats[2], w: floats[3]),
                   .init(x: floats[4], y: floats[5], z: floats[6], w: floats[7]),
                   .init(x: floats[8], y: floats[9], z: floats[10], w: floats[11]),
                   .init(x: floats[12], y: floats[13], z: floats[14], w: floats[15]))
    }
    ...
    public static func + (lhs: simd_float4x4, rhs: simd_float4x4) -> simd_float4x4 {
        let lc = lhs.columns
        let rc = rhs.columns
        let col0 = simd_float4(x: lc.0.x + rc.0.x, y: lc.0.y + rc.0.y, z: lc.0.z + rc.0.z, w: lc.0.w + rc.0.w)
        let col1 = simd_float4(x: lc.1.x + rc.1.x, y: lc.1.y + rc.1.y, z: lc.1.z + rc.1.z, w: lc.1.w + rc.1.w)
        let col2 = simd_float4(x: lc.2.x + rc.2.x, y: lc.2.y + rc.2.y, z: lc.2.z + rc.2.z, w: lc.2.w + rc.2.w)
        let col3 = simd_float4(x: lc.3.x + rc.3.x, y: lc.3.y + rc.3.y, z: lc.3.z + rc.3.z, w: lc.3.w + rc.3.w)
    ...
    public static func * (lhs: simd_float4x4, rhs: Float) -> simd_float4x4 {
        let lc = lhs.columns
        let col0 = simd_float4(x: lc.0.x * rhs, y: lc.0.y * rhs, z: lc.0.z * rhs, w: lc.0.z * rhs)
        let col1 = simd_float4(x: lc.1.x * rhs, y: lc.1.y * rhs, z: lc.1.z * rhs, w: lc.1.z * rhs)
        let col2 = simd_float4(x: lc.2.x * rhs, y: lc.2.y * rhs, z: lc.2.z * rhs, w: lc.2.z * rhs)
        let col3 = simd_float4(x: lc.3.x * rhs, y: lc.3.y * rhs, z: lc.3.z * rhs, w: lc.3.z * rhs)
        return .init(col0, col1, col2, col3)
    }
